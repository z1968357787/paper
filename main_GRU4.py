...
class NeuralNetwork(nn.Module):
    def __init__(self, input_size,hidden_size=64):
        ...
        self.temp = nn.GRU(input_size, hidden_size, 1,batch_first=True)
    ...
    def forward(self, x,hidden):#forward就是专门用来计算给定输入，得到神经元网络输出的方法

        output,hidden=self.temp(x,hidden)
...
if __name__ == '__main__':
    ...
    optimizer = torch.optim.SGD(model.parameters(), lr=1e-4)#定义SGD随机梯度下降法，学习率
    ...
    model.eval()
    with torch.no_grad():
        y_pred_test,hidden = model(x_test,hidden)
